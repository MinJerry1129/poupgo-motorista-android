...
public class SelectedDayHistoryActivity extends AppCompatActivity {
    ...
    public void setData(boolean isDataAvail, String responseString) {
        ...
        if (isDataAvail) {
            ...
            if (msgArr != null) {
                ...
                for (int i = 0; i < msgArr.length(); i++) {
                    ...
                    if (generalFunc.getJsonValue("eHailTrip", obj_temp.toString()).equalsIgnoreCase("Yes")) {
                        if (generalFunc.getJsonValue("eRental", obj_temp.toString()) != null && generalFunc.getJsonValue("eRental", obj_temp.toString()).equalsIgnoreCase("Yes")) {

                            ((MTextView) customView.findViewById(R.id.typeTxt)).setText(generalFunc.retrieveLangLBl("Hail", "LBL_HAIL") +
                                    " - " + generalFunc.retrieveLangLBl("", "LBL_RENTAL_CATEGORY_TXT"));
                        } else {
                            ((MTextView) customView.findViewById(R.id.typeTxt)).setText(generalFunc.retrieveLangLBl("Hail", "LBL_HAIL"));
                        }
           ...
           // JSONArray canclemsgArr = generalFunc.getJsonArray(CommonUtilities.message_str_one, responseString);
//            for (int i = 0; i < canclemsgArr.length(); i++) {
//                JSONObject obj_temp = generalFunc.getJsonObject(canclemsgArr, i);
//
//                LayoutInflater inflater = (LayoutInflater) getActContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
//                View customView = inflater.inflate(R.layout.selected_day_trip_history_item, null);
//
...
//                ((ImageView) customView.findViewById(R.id.arrowImgView)).setColorFilter(Color.parseColor("#2F2F2F"));
//
//                (customView.findViewById(R.id.tripItem)).setOnClickListener(new setOnClickList(true, i, true));
//                listCancelContainer.addView(customView);
//
//                cancel_list_item.add(obj_temp.toString());
//
//            }
